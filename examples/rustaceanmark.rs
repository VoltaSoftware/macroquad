...
async fn main() {
    ...
    loop {
        ...
        if macroquad::input::is_mouse_button_down(MouseButton::Left) {
            for _i in 0..100 {
                rustaceanes.push(Rustaceane {
                    ...
                    speed: Vec2::new(rand::gen_range(-250., 250.) / 60., rand::gen_range(-250., 250.) / 60.),
                    color: Color::from_rgba(rand::gen_range(50, 240), rand::gen_range(80, 240), rand::gen_range(100, 240), 255),
                })
            }
        }
        ...
        for rustaceane in &mut rustaceanes {
            ...
            if ((rustaceane.pos.x + rustacean_tex.width() / 2.) > screen_width()) || ((rustaceane.pos.x + rustacean_tex.width() / 2.) < 0.)
            {
