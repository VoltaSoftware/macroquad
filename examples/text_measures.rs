...
fn draw_text_annotated(text: &str, font: Option<&Font>, x: f32, baseline: f32) {
    ...
    draw_text("baseline", x + size.width + 10.0, baseline - 5.0, 30.0, WHITE);

    // Top line
    draw_rectangle(x, baseline - 2.0 - size.offset_y, size.width, 4.0, RED);

    // Top line annotation
    draw_rectangle(x + size.width, baseline - size.offset_y - 1.0, 120.0, 1.0, GRAY);
    draw_text("topline", x + size.width + 10.0, baseline - size.offset_y - 5.0, 30.0, WHITE);

    // Bottom line
    draw_rectangle(x, baseline - 2.0 - size.offset_y + size.height, size.width, 4.0, RED);

    // Bottom line annotation
    draw_rectangle(x + size.width, baseline - size.offset_y + size.height - 1.0, 120.0, 1.0, GRAY);
    draw_text(
        "bottomline",
        x + size.width + 10.0,
